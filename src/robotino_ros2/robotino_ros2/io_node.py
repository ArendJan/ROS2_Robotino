...
class IONode(Node):
    ...
    def distance_callback(self):
        try:
            result = requests.get(f"http://{self.ip}/data/distancesensorarray").json()
            msg = DistanceArray()
            msg.array = result
    ...
    def digital_output_control(self, msg):
        ...
        requests.post(f"http://{self.ip}/data/digitaloutput", data=json.dumps(request_json))

    def degital_output_array_control(self, msg):
        request_json = msg.array
        if len(request_json) == 8:
            requests.post(f"http://{self.ip}/data/digitaloutputarray",
    ...
    def relay_control(self, msg):
        ...
        requests.post(f"http://{self.ip}/data/relay", data=json.dumps(request_json))

    def relay_array_control(self, msg):
        request_json = msg.array
        if len(request_json) == 8:
            requests.post(f"http://{self.ip}/data/relayarray",
                          data=json.dumps(request_json))

    def get_io_status(self, request, response):
        result = requests.get(f"http://{self.ip}/data/analoginputarray").json()
        response.analog_input_array = result
        result = requests.get(f"http://{self.ip}/data/digitalinputarray").json()
        response.digital_input_array = result
        result = requests.get(f"http://{self.ip}/data/digitaloutputstatus").json()
        response.digital_output_array = result
        result = requests.get(f"http://{self.ip}/data/relaystatus").json()
